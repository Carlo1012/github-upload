...
include '../include/core.inc.php'; 
 if (isset($_REQUEST['page'])) {
  ...
  if($_REQUEST['page']=='delete') {
    ...
    header("refresh:1 url=reportlist_admin.php?r=deleted");
    
    }elseif($_REQUEST['page']=='revise') {
    $query = "UPDATE uploaded_files SET status = '2' WHERE id = '".$_REQUEST['id']."' ";
    mysqli_query($conn, $query);
    header("refresh:1 url=reportlist_admin.php?r=updated");
    
    }elseif($_REQUEST['page']=='deleted') {
    $query = "DELETE FROM uploaded_files WHERE id = '".$_REQUEST['id']."' ";
    mysqli_query($conn, $query);
    header("refresh:1 url=all_logs.php?r=deleted");
